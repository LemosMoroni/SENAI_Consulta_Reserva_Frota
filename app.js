...
/** Filtra por nome do motorista e/ou cidade de destino (e data opcional) e renderiza os cards */
function filtrarEExibir(dados, nomeBusca, dataBusca, cidadeBusca) {
  ...
  const resultado = dados.filter((row, i) => {
    ...
    // Filtro por cidade de destino (parcial, sem acento)
    if (normCidade) {
  ...
  const termoBusca = [nomeBusca, cidadeBusca].filter(Boolean).join(' / ');

  if (resultado.length === 0) {
    $('nomeNaoEncontrado').textContent = dataBusca
      ? `${termoBusca} na data selecionada`
      : termoBusca;
    mostrar('emptyState');
    return;
  }
  ...
  $('resultCount').innerHTML =
    `<strong>${resultado.length}</strong> reserva(s) para <strong>${termoBusca}</strong>${labelData}`;
...
function dadosDemo() {
  ...
  return [
    ['SEN-000001/2026', 'QHA7788 - Uno Evolution [SENAI]',  '', 'Carlos Eduardo Mendes', '30/03/2026, 07:00', '30/03/2026, 18:00', 'SENAI Lages - 049', 'Manhã / Tarde',  'Carlos', 'Lages'],
    ['SEN-000002/2026', 'TPO5E39 - ONIX [SENAI]',           '', 'Ana Paula Oliveira',    '31/03/2026, 13:00', '31/03/2026, 22:00', 'SENAI Lages - 049', 'Tarde / Noite',  'Ana', 'Florianópolis'],
    ['SEN-000003/2026', 'RXL1L31 - HB20 [SENAI]',           '', 'Marcos Antônio Costa',  '01/04/2026, 07:00', '01/04/2026, 18:00', 'SENAI Lages - 049', 'Manhã / Tarde',  'Marcos', 'Florianópolis'],
  ];
}
