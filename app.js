...
async function buscar() {
  ...
  const cidadeBusca = $('inputCidade').value.trim();

  if (!nomeBusca && !cidadeBusca) { inputNome.focus(); return; }
  ...
  try {
    ...
    filtrarEExibir(dados, nomeBusca, dataBusca, cidadeBusca);
  } catch (err) {
    console.warn('Planilha indisponível, usando dados demo:', err.message);
    filtrarEExibir(dadosDemo(), nomeBusca, dataBusca, cidadeBusca);
  } finally {
    setLoading(false);
  }
}
...
/** Filtra por nome, cidade de destino e/ou data, e renderiza os cards */
function filtrarEExibir(dados, nomeBusca, dataBusca, cidadeBusca) {
  const normBusca   = norm(nomeBusca);
  const normCidade  = norm(cidadeBusca);
  const { col } = CONFIG;
  ...
  const resultado = dados.filter((row, i) => {
    ...
    // Filtro por cidade de destino (se preenchida)
    if (cidadeBusca) {
      const cidadeCell = row[col.cidadeDestino] || '';
      if (!norm(cidadeCell).includes(normCidade)) return false;
    }
  ...
  if (resultado.length === 0) {
    let msgVazia = '';
    if (nomeBusca && cidadeBusca) msgVazia = `${nomeBusca} com destino a ${cidadeBusca}`;
    else if (nomeBusca)           msgVazia = nomeBusca;
    else                          msgVazia = `destino ${cidadeBusca}`;
    if (dataBusca) msgVazia += ' na data selecionada';
    $('nomeNaoEncontrado').textContent = msgVazia;
    mostrar('emptyState');
    return;
  }

  let labelResultado = '';
  if (nomeBusca)   labelResultado += ` para <strong>${nomeBusca}</strong>`;
  if (cidadeBusca) labelResultado += ` com destino a <strong>${cidadeBusca}</strong>`;
  if (dataBusca)   labelResultado += ` em <strong>${dataBusca.split('-').reverse().join('/')}</strong>`;
  $('resultCount').innerHTML = `<strong>${resultado.length}</strong> reserva(s)${labelResultado}`;
  $('resultList').innerHTML  = resultado.map((row, i) => renderCard(row, i)).join('');
...
function dadosDemo() {
  // [solicitacao, veiculo, localizador, motorista, dataInicial, dataFinal, local, turno, ocupantes, cidadeDestino]
  return [
    ['SEN-000001/2026', 'QHA7788 - Uno Evolution [SENAI]',  '', 'Carlos Eduardo Mendes', '30/03/2026, 07:00', '30/03/2026, 18:00', 'SENAI Lages - 049', 'Manhã / Tarde',  'Carlos',  'Florianópolis'],
    ['SEN-000002/2026', 'TPO5E39 - ONIX [SENAI]',           '', 'Ana Paula Oliveira',    '31/03/2026, 13:00', '31/03/2026, 22:00', 'SENAI Lages - 049', 'Tarde / Noite',  'Ana',     'Blumenau'],
    ['SEN-000003/2026', 'RXL1L31 - HB20 [SENAI]',           '', 'Marcos Antônio Costa',  '01/04/2026, 07:00', '01/04/2026, 18:00', 'SENAI Lages - 049', 'Manhã / Tarde',  'Marcos',  'Joinville'],
  ];
}
...
document.addEventListener('DOMContentLoaded', () => {
  ['inputNome', 'inputCidade'].forEach(id => {
